...
import Experiences.{DistributedGA, SingleGA}
...
object GAOperations {
	...
	def tournamentSelectionAndUniformCrossover(broadPopulation:Broadcast[Array[BNStructure]],tournamentSize:Int,numOfPopulation:Int,sc:SparkContext):RDD[BNStructure] = {
		...
		//对TSIRDD中每一个TSI数组，随机在广播的TSI数组中选取两条TSI，与其合并为TSI组，即一个TSI组包含三条TSI，方便后续做演化操作，每个TSI组对应一个后代
		...
		TSIGroupRDD.flatMap(eachTSIGroup =>{
			...
			//对G1、G2进行交叉，并合并G0
			Array(G0,new BNStructure(uniformCrossover(G1.structure,G2.structure,DistributedGA.numOfAttributes)))
		})
	}
